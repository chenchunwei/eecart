...
}})(jQuery);$(function(){var a=$('[data-event="scroll"]');a.each(function(b,f){var h=$(this);var g=h.attr("data-mum")>>>0;if(h.find("img").length!=0){var c=h.find(".next"),e=h.find(".prev");
var d=h.children("div");d.jCarouselLite({btnNext:c,btnPrev:e,visible:g,scroll:1,speed:350,auto:4000,mouseOver:true});}else{h.jCarouselLite({vertical:true,visible:g,scroll:1,speed:350,auto:3000,mouseOver:true});
}});});(function(b){b.fn.lazyload=function(a){var h={attr:"data-url"};var g=b.extend({},h,a||{});g.cache=[];b(this).each(function(){var d=this.nodeName.toLowerCase(),e=b(this).attr(g.attr);
if(!e){return;}var c={obj:b(this),tag:d,url:e};g.cache.push(c);});var f=function(){var d=b(window).scrollTop(),c=d+b(window).height();b.each(g.cache,function(n,i){var e=i.obj,p=i.tag,o=i.url;
if(e){post=e.offset().top;posb=post+e.height();if((post>d&&post<c)||(posb>d&&posb<c)){if(p==="img"){e.attr("src",o);}else{e.load(o);}i.obj=null;}}});return false;
};f();b(window).bind("scroll",f);};})(jQuery);$(function(){$("img").lazyload();});(function(a){a.fn.sallytoggle=function(b){var c={};var b=a.extend(c,b);
this.each(function(){var g=a(this);var d=g.attr("data-target");var e=g.attr("data-event");var f={close:function(){g.bind("click",function(h){a(this).parents("."+d).fadeOut();
h.preventDefault();h.stopPropagation();});},hover:function(){g.bind("mouseenter",function(){clearTimeout(g.ctimes);g.addClass(d);}).bind("mouseleave",function(){clearTimeout(g.ctimes);
g.ctimes=setTimeout(function(){g.removeClass(d);},0);});},dropdown:function(){var h=g.children("."+d);g.bind("click.dropdown",function(i){h.toggle();
